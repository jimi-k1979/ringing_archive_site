...
self.location='drl_stat_home.php?tg=' + val ;
}
...
function enable_fields() {
	...
	document.team_details.start.value=1972;
...
function check_form (form) {
	...
	// - Both boxes must be numbers greater than 1986 and end should be greater than or equal to start
	...
	if(flag == -1) { // error of no radio button checked - shouldn't get here!
		...
		if (isNaN(start_year) || isNaN(end_year)) { // cheeck for non numerical input
			...
			return false;
		} else if (start_year<1972 || end_year<start_year) { // check that the numbers are in the right range
			window.alert("The start year must be 1972 or later\nand the end year must not be before the end year");
			return false;
...
<div class='text'>Using the form below you can select to view the statistics and/or results for every season from 1986 for any tower that has entered a competition since then. The towers are ordered as they would be in 'Dove' so, for example, you'll find Eggbuckland under 'P' for Plymouth and Alphington under 'E' for Exeter.</div>
<?php	
